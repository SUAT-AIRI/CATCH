...
def gen_dstc_text(args):
    ...
    with open(args.dataset, 'r') as f1:
        for line in f1:
            ...
            for turn in dial_data['turns']:
                dials.append(turn['utterance'])
                themes.append(turn['theme_label'])
            dial_len = len(dials)
            for utt_idx in range(dial_len-1):
                ...
                negdial = random.choice(picked_dials)
                ...
                for i in range(args.history):
                    ...
                    if neg_index < dial_len:
                        neg.append(re.sub(r'\s([,?.!"](?:\s|$))', r'\1', dials[l-1]))
                        neg_index += 1
    ...
    # indices = torch.randperm(len(data))[:10000]
    # select_data = [data[i] for i in indices]
    # select_topic_data = [topic_data[i] for i in indices]
    json.dump(data, open(f'../data/dstc12.json', 'w'))
    json.dump(topic_data, open(f'../data/dstc12_topic_data.json', 'w'))


def main(args):
    MAX_LEN = 512
    for dataset in tqdm(['TIAGE']):
        data = json.load(open(f'../data/dstc12.json'))
        topic_data = json.load(open(f'../data/dstc12_topic_data.json'))
